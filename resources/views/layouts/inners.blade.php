...
<head>
    ...
    <link rel="stylesheet" href="{{url('')}}/themes/porto/css/theme-shop.css">
    <style TYPE="text/css">
        .modal2{
            margin-left: -50px;
        }
    ...
    $(window).load(function() {
        ...
        var validator = $('#registerForm').validate({
            ...
            errorPlacement: function (error, element) {
                ...
                $("#myModalLogin").modal("hide")
                $("#myProcess").modal("show")
                $.ajax({url: '{{url("")}}/orderform/{{$bag_title}}',type: 'post',data: $(form).serialize(),
                    success:function(data){if(data){$("div#transProcess").html("<div class='alert alert-info fade in'><button class='close' data-dismiss='alert'>×</button><i class='fa-fw fa fa-check'></i>"+data+"</div>")}else{alert(data);}setInterval($("#myProcess").modal("hide"),500000);}});
            }
        });
...
//function to format bites bit.ly/19yoIPO
        ...
        function loadPricingData(){
            ...
            var prod = $("#item").val();
            var href = location.href; //get current url for this page

            //Return price string from ajax
            $.ajax({url:href,type:"post",data:{_token: $("#_token").val(),bsize:$("#bag_size").val(),qty:$("#quantity").val(),prod:prod,faction:"getprice"},dataType: 'html',
                success:function(data){
                    // remove all quotes from around the return string
                    var amount = parseFloat(data.replace('"',"")) * qty
                    //check if amount is a valid numeric
                    if(!isNaN(amount)){
                        $("#price").val(numeral(amount).format('0,0.00'))
                        $(".amount").html(numeral(amount).format('0,0.00'))
                        $(".subtotal").html(numeral(amount).format('0,0.00'))
                    }else{
                        $("#price").val(0.00)
                        $(".amount").html(0.00)
                        $(".subtotal").html(0.00)
